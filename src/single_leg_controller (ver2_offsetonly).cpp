...
SingleLegController::SingleLegController() : nh_("~") {
    ...
    nh_.param("joint_zero_position/coxa_offset", coxa_offset_temp, 2048);
    nh_.param("joint_zero_position/femur_offset", femur_offset_temp, 2048);
    nh_.param("joint_zero_position/tibia_offset", tibia_offset_temp, 2048);
...
uint32_t SingleLegController::angleToPosition(double angle, uint32_t offset) {
    // 角度を-πからπの範囲に正規化
...
double SingleLegController::positionToAngle(uint32_t position, uint32_t offset) {
